"""Conditional panel associations with calendar-aligned outcomes and lags.
Country/year effects; clustered covariance or a Bartlett Driscoll-Kraay check.
These regressions do not identify structural innovations or policy effects.
"""
...
def panel_ols(
    ...
    if covariance == "dk":
        scores = pd.DataFrame(X * resid[:, None]).groupby(d[time]).sum().sort_index()
        S = scores.to_numpy()
        meat = S.T @ S
        for lag in range(1, min(bandwidth, len(S) - 1) + 1):
            cross = S[lag:].T @ S[:-lag]
    ...
    p = 2 * stats.t.sf(
        np.abs(t),
        df=(d[time].nunique() - 1 if covariance == "dk" else d[entity].nunique() - 1),
    )
...
def estimate_state_equation(panel: pd.DataFrame) -> dict:
    ...
    d["logitM_lead"] = calendar_shift(d, "logitM", -1)
    d["stress"] = d.groupby("iso3")["npl_i"].transform(
        lambda s: (s - s.mean()) / (s.std() if s.std() > 0 else 1.0)
    )
    d["infl_z"] = (
        d["inflation"].clip(-10, 60) - d["inflation"].clip(-10, 60).mean()
    ...
    return {
        ...
        "avg_T": float(T),
        "n_obs": int(tab.attrs["n_obs"]),
        "n_entities": int(tab.attrs["n_entities"]),
    }
...
CHANNELS = {
    ...
    "I": ("gini", "diff"),  # observed endpoint change in Gini
    "T": ("export_growth", "cum"),  # cumulative export growth
    "F": ("npl", "diff"),  # observed endpoint change in NPL ratio
}


def calendar_shift(d: pd.DataFrame, var: str, periods: int) -> pd.Series:
    """Value at year t-periods; never treat a gap as a consecutive year."""
    lookup = d.set_index(["iso3", "year"])[var]
    if lookup.index.has_duplicates:
        raise ValueError("Duplicate country-year observations")
    keys = pd.MultiIndex.from_arrays([d.iso3, d.year - periods])
    return pd.Series(lookup.reindex(keys).to_numpy(), index=d.index)


def past_fill(d: pd.DataFrame, var: str, max_age: int) -> pd.Series:
    """Carry a past value only while its calendar age is at most max_age."""
    last_year = d.year.where(d[var].notna()).groupby(d.iso3).ffill()
    value = d.groupby("iso3")[var].ffill()
    return value.where(d.year - last_year <= max_age)


def _lp_outcome(d: pd.DataFrame, var: str, h: int, kind: str) -> pd.Series:
    if h < 0:
        if kind == "cum":
            return sum(calendar_shift(d, var, -j) for j in range(h, 0))
        # Past changes over |h| periods, ending at t-1 (nondegenerate at h=-1).
        return calendar_shift(d, var, 1) - calendar_shift(d, var, 1 - h)
    if kind == "cum":
        return sum(calendar_shift(d, var, -j) for j in range(h + 1))
    return calendar_shift(d, var, -h) - calendar_shift(d, var, 1)
...
def local_projections(
    ...
    # Predetermined conditioning states W = (F, T, I) at calendar t-1.
    for src, name in [("npl_i", "W_F"), ("trade_gdp", "W_T"), ("gini_i", "W_I")]:
        d[name] = calendar_shift(d, src, 1)
        d[name] = (d[name] - d[name].mean()) / d[name].std()
    ...
    if include_history:
        d["M_lag"] = calendar_shift(d, "M_z", 1)
        xvars = xvars + ["M_lag"]
    rows = []
    for k, (var, kind) in CHANNELS.items():
        for h in horizons:
            d["_y"] = _lp_outcome(d, var, h, kind)
            d["outcome_lag"] = calendar_shift(d, var, 1)
            ...
            if h >= 0 and k in ("N", "T"):
                xv = xv + ["outcome_lag"]
            tab = panel_ols(d, "_y", xv, covariance=covariance)
            for v in xv:
                rows.append(
                    {
                        "channel": k,
                        ...
                        "p": tab.loc[v, "p"],
                        "n": tab.attrs["n_obs"],
                        "countries": tab.attrs["n_entities"],
                        "cov_M_F": tab.attrs["cov"][xv.index("M_z"), xv.index("MxW_F")]
                        if interactions
                        else 0.0,
                    }
                )
...
def quantile_lp(panel, quantiles=(0.1, 0.25, 0.5, 0.75, 0.9), h=1, n_boot=399):
    from .validation import quantile_inference

    return quantile_inference(panel, quantiles, h, n_boot)[0]
...
def iv_growth(panel: pd.DataFrame, h: int = 1) -> dict:
    ...
    d = panel.sort_values(["iso3", "year"]).copy()
    from .data import fetch_wb_indicator

    net = fetch_wb_indicator("IT.NET.USER.ZS", start=2000, end=2004).rename(
        columns={"value": "internet"}
    )
    w = net[net.year <= 2004].groupby("iso3")["internet"].mean() / 100.0
    d["exposure"] = d["iso3"].map(w)
    d = d[d.year >= 2005].copy()
    d["M_z"] = (d["M"] - d["M"].mean()) / d["M"].std()
    ...
    mhat = z @ pi
    first_stage = panel_ols(dd, "M_z", ["spill_lag", "infl_z", "growth_lag"])
    fstat = first_stage.loc["spill_lag", "t"] ** 2
    ...
    return {
        "beta_M": float(beta[0]),
        "se_M": float(np.sqrt(V[0, 0])),
        "first_stage_F": float(fstat),
        "n": len(y),
    }
