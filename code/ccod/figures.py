...
def fig_index_trends(panel: pd.DataFrame):
    ...
    ax.fill_between(
        wm.index, q25, q75, color=C_BASE, alpha=0.18, lw=0, label="interquartile range"
    )
    ...
    for c, reg in zip(colors, order):
        ax.plot(regs.index, regs[reg], lw=1.3, color=c, label=shorten.get(reg, reg))
...
def fig_lp_irfs(lp: pd.DataFrame):
    ...
    axes[0].set_ylabel("association with +1 s.d. $M$")
    fig.tight_layout()
...
def fig_amplification(lp: pd.DataFrame):
    ...
    inter = {
        "MxW_F": "× financial stress",
        "MxW_T": "× trade exposure",
        "MxW_I": "× inequality",
    }
    fig, axes = plt.subplots(1, 2, figsize=(8.6, 3.0))
    ...
    for c, (v, lab) in zip(colors, inter.items()):
        ...
        ax.errorbar(
            d.h + {"MxW_F": -0.12, "MxW_T": 0, "MxW_I": 0.12}[v],
            d.coef,
            yerr=1.645 * d.se,
            fmt="o",
            ms=4,
    ...
    # right: conditional slope of M for growth at low/mid/high stress, h=2
    ax = axes[1]
    ...
    tot = b + phi * ws
    se = np.sqrt(np.maximum(0, se_b**2 + (ws * se_p) ** 2 + 2 * ws * d.cov_M_F.iloc[0]))
    ax.fill_between(
        ws, tot - 1.645 * se, tot + 1.645 * se, alpha=0.18, color=C_HIGH, lw=0
    )
    ax.plot(ws, tot, color=C_HIGH, lw=1.8)
    ax.axhline(0, color=C_NEUT, lw=0.8)
    ax.set_title("B. Growth association with $M$ by financial stress ($h{=}2$)")
...
def fig_growth_at_risk(qlp: pd.DataFrame):
    ...
    ax.set_ylabel("association with +1 s.d. $M$ (pp)")
    ax.set_title("Disinformation and conditional growth quantiles")
    fig.tight_layout()
...
def fig_tipping(psi_grid: dict, bifurcation: pd.DataFrame):
    ...
    ax.axvline(bifurcation.attrs.get("phi_crit", np.nan), color=C_HIGH, lw=0.9, ls="--")
    ax.annotate(
        "escalation\nthreshold $\\varphi^{*}$",
...
def fig_policy(policy: pd.DataFrame):
    ...
    labels = {
        "rho": "rapid correction ($\\rho\\downarrow$)",
        "c": "platform friction ($c\\downarrow$)",
        "sigma": "verified messaging ($\\sigma_u\\downarrow$)",
    }
    for lever, d in policy.groupby("lever"):
        ax.plot(d["effort"], d["dD"], color=colors[lever], lw=1.8, label=labels[lever])
    ax.axhline(100, color=C_NEUT, lw=0.8, ls="--")
    ax.annotate(
        "full elimination of baseline burden", (0.005, 103), fontsize=7, color=C_NEUT
    )
    ax.set_ylim(-5, 185)
