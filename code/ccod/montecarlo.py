...
(iii) the interaction (amplification) coefficient phi. This tests numerical recovery under the chosen stylized DGP, not the
empirical design's identification or a tail-concentration theorem.
"""
...
from .estimation import panel_ols
from .simulate import sigmoid


def simulate_panel_dgp(
    R=150, T=24, rho=0.85, c=0.0, sigma_u=0.5, beta=-0.4, phi=-0.5, seed=1
) -> pd.DataFrame:
    ...
    for r in range(R):
        ...
        for t in range(T):
            ...
            M = sigmoid(x)
            rows.append(
                {"iso3": f"C{r:03d}", "year": 2000 + t, "M": M, "logitM": x, "F": F}
            )
    d = pd.DataFrame(rows)
