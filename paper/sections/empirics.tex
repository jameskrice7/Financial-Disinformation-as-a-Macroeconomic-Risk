...
\input{tables/state_table}
The state equation uses \StateN\ observations from
...
$\rho=0.99$ would imply about 69 years. Equation~\eqref{eq:half-sensitivity}
explains the sensitivity of this transformation near unity.
...
At one standard deviation above mean banking stress, their sum is about
$-2.46$ points; at one standard deviation below it, their difference is
about $-0.58$ points, keeping other conditioning states at their reference
values. These are evaluations of the fitted slope per standardized index
unit, not predicted growth levels for particular countries. Their uncertainty
depends on the joint covariance of the two estimates. The threshold
aggregation in Proposition~\ref{prop:aggregation} supplies a possible
explanation for the sign of this stress gradient, while leaving its scale
and causal interpretation to additional measurement and identification.
...
quantile regressions on the specified covariates and an intercept, following
...
The implementation uses a linear-programming quantile solver and
...
fully included.
\input{tables/quantile_table}
\input{tables/quantile_contrasts}
...
slope intervals include zero. The data therefore do not support a statistically
distinct lower-tail association. The location-scale example in
Section~\ref{subsec:aggregation} shows why this finding need not contradict
amplification conditional on banking stress.
\begin{figure}[!htbp]\centering
...
A regional-pressure instrument interacted with internet penetration would
require an exclusion restriction beyond the fixed effects and controls.
Internet exposure may affect growth through other channels, and regional
pressure may reflect regional shocks. Exposure measured over 2000--2004 is
also not predetermined for observations within that window. Instrument
relevance alone would not resolve these concerns. The empirical conclusions
therefore rest on conditional associations rather than instrumental-variable
identification.
